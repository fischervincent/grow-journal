...
function HeaderButtons({ callbackURL }: { callbackURL: string | null }) {
  return (
    <>
      <Link
        href={{
...
function MobileMenu({ callbackURL }: { callbackURL: string | null }) {
  return (
    <div className="container md:hidden border-t bg-white">
      <div className="flex flex-col gap-4 p-4">
        ...
        >
          <Button
            variant="outline"
            className="border-green-200 text-green-700 w-full"
          >
            Sign In
          </Button>
        </Link>
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-white">
      <div className="container flex h-16 items-center justify-between px-4 md:px-6">
        ...
        <div className="hidden md:flex gap-4">
          <HeaderButtons callbackURL={callbackURL} />
        </div>
        <Button
          variant="ghost"
          size="icon"
      ...
      {isMenuOpen && <MobileMenu callbackURL={callbackURL} />}
    </header>
  );
}
